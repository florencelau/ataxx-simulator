...
class Move {
    ...
    /** Return an INFTYMOVE. */
    static Move inftyMove() {
        ifInfty = true;
        return INFTYMOVE;
    }

    /** Return a NEGINFTYMOVE. */
    static Move negInftyMove() {
        ifNegInfty = true;
        return NEGINFTYMOVE;
    }

    /** Return true iff I am an INFTYMOVE. */
    boolean isInftyMove() {
        return ifInfty;
    }

    /** Return true iff I am a NEGINFTYMOVE. */
    boolean isNegInftyMove() {
        return ifNegInfty;
    }
    ...
    boolean isExtend() {
        return (findDistance(col1(), col0()) == 1
                || findDistance(row1(), row0()) == 1);
    }

    /** Return true if thIs is a jump (move to adjacent square). */
    boolean isJump() {
        return (findDistance(col1(), col0()) == 2
                || findDistance(row1(), row0()) == 2);
    }

    /** Return distance between COL and ROW. */
    int findDistance(int col, int row) {
    ...
    public String toString() {
        if (isPass()) {
            ...
            String result = "";
            result = result + col0() + row0() + '-' + col1() + row1();
            return result;
        }
    }
    ...
    /** Type of artificial move with a static score
     * of infinity.
     */
    static final Move INFTYMOVE = new Move();

    /** Type of artificial move with a static score
     * of negative infinity.
     */
    static final Move NEGINFTYMOVE = new Move();
    ...
    /** Return true iff this move has a
     * static score of infinity.
     */
    private static boolean ifInfty = false;

    /** Return true iff this move has a
     * static score of negative infinity.
     */
    private static boolean ifNegInfty = false;

    /** The set of all Moves other than pass,
     * indexed by from and to row and
     *  column positions. */
